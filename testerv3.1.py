#!/usr/bin/env python3
"""
Automated Host Testing Script - Linux/Ubuntu Only
Author: Haran Muru
"""
...
class HostTester:
    def __init__(self, logPath="./screeningResults"):
        # Initialize the host tester with configuration and setup logging
        ...
        # Fixed benchmark duration - 3.75 minutes
        self.benchmarkDurationMinutes = 3.75
        
        # Temperature thresholds
        self.TEMP_WARNING_THRESHOLD = 87.5  # °C
    ...
    def getMoboName(self):
        # Get motherboard product name using dmidecode
        try:
    ...
    def verifyToolPaths(self):
        # Check if required diagnostic tools exist at specified paths
        # Check DGDiagTool
    ...
    def detectCpuVendor(self):
        # Identify CPU vendor (Intel/AMD) from /proc/cpuinfo
        try:
    ...
    def getCpuSpeedLimits(self):
        # Extract CPU min/max frequency limits from lscpu output
        try:
    ...
    def runDgDiag(self, command, timeout=30):
        # Execute DG diagnostic tool commands with error handling and timeout
        dgDiagExe = self.dgDiagPath / "DGDiagTool"
    ...
    def getAllCpuTemperatures(self):
        # Get CPU temperature readings based on detected vendor (Intel/AMD)
        try:
    ...
    def getAllIntelTemps(self):
        # Read Intel CPU temperatures using lm-sensors and coretemp
        temperatures = {}
    ...
    def getAllAmdTemps(self):
        # Read AMD CPU temperatures (Tctl/Tccd) using k10temp sensor
        temperatures = {}
    ...
    def getHwmonTemperatures(self):
        # Read temperatures directly from hwmon interface as fallback
        temperatures = {}
    ...
    def getGenericTemps(self):
        # Read temperatures from generic thermal zones as last resort
        temperatures = {}
    ...
    def getCpuFrequencies(self):
        # Get current CPU frequencies for all cores using psutil or /proc/cpuinfo
        try:
            # Try psutil first
    ...
    def startMonitoring(self):
        # Start background thread to monitor CPU speeds and temperatures
    ...
    def stopMonitoring(self):
        # Stop background monitoring thread and wait for completion
        self.monitoringActive = False
    ...
    def monitoringLoop(self):
        # Background loop that collects CPU speed and temperature data every second
    ...
    def startBackgroundTimer(self):
        """Start background timer notifications for 3.75 minutes"""
        try:
            # Create the timer script content for 3 minutes 45 seconds
            timer_script = '''#!/bin/bash
{
    notify-send "Host Test Started" "CPU benchmark will run for 3 minutes 45 seconds" 2>/dev/null
    sleep 60
    notify-send "Host Test Progress" "2 minutes 45 seconds remaining" 2>/dev/null
    sleep 60
    notify-send "Host Test Progress" "1 minute 45 seconds remaining" 2>/dev/null
    sleep 60
    notify-send "Host Test Progress" "45 seconds remaining" 2>/dev/null
    sleep 45
    notify-send "Host Test Complete" "CPU benchmark finished successfully!" 2>/dev/null
} &
'''
            
            # Execute the timer script
            subprocess.Popen(['bash', '-c', timer_script])
            self.logger.info("Started background timer notifications")
            
        except Exception as e:
            self.logger.warning(f"Could not start background timer: {e}")
    
    def runCpuBenchmark(self):
        # Execute mprime CPU stress test for configured duration with monitoring
        ...
        try:
            ...
            # Wait for set minutes (3.75 minutes = 225 seconds)
    ...
    def testPcieSlots(self):
        # Test PCIe slot performance by comparing current vs maximum speed/width
    ...
    def evaluateTemperatures(self, tempSummary):
        """
        Analyze temperature readings against warning/critical thresholds for pass/fail status
        Checks both maximum AND average temperatures with appropriate thresholds
        """
        tempStatus = {
            'overall': 'PASS',
            'warnings': [],
        ...
        self.logger.info("Evaluating temperature readings (both max and average)...")
        
        # Define average temperature thresholds (slightly lower for sustained temps)
        AVG_TEMP_WARNING = self.TEMP_WARNING_THRESHOLD - 2.5   # 85.0°C
        AVG_TEMP_CRITICAL = self.TEMP_CRITICAL_THRESHOLD - 2.5  # 87.5°C
        
        # Calculate overall maximum average temperature
        allAvgTemps = [tempData['averageC'] for tempData in tempSummary.values()]
        tempStatus['maxAvgTemp'] = max(allAvgTemps) if allAvgTemps else 0
        
        for sensorName, tempData in tempSummary.items():
            maxTemp = tempData['maxC']
            avgTemp = tempData['averageC']
            minTemp = tempData['minC']
            
            tempStatus['maxTemp'] = max(tempStatus['maxTemp'], maxTemp)
            
            # Store detailed sensor info
            tempStatus['sensorDetails'][sensorName] = {
                'max': maxTemp,
                'avg': avgTemp,
                'min': minTemp,
                'maxStatus': 'OK',
            ...
            # Evaluate maximum temperatures
            if maxTemp >= self.TEMP_CRITICAL_THRESHOLD:
                tempStatus['overall'] = 'FAIL'
                tempStatus['critical'].append(f"{sensorName} MAX: {maxTemp}°C")
                tempStatus['criticalSensors'].append(sensorName)
                tempStatus['maxTempIssues'].append(f"{sensorName}: {maxTemp}°C")
                tempStatus['sensorDetails'][sensorName]['maxStatus'] = 'CRITICAL'
                self.logger.error(f"✗ CRITICAL MAX: {sensorName} peaked at {maxTemp}°C (threshold: {self.TEMP_CRITICAL_THRESHOLD}°C)")
                
            elif maxTemp >= self.TEMP_WARNING_THRESHOLD:
                if tempStatus['overall'] != 'FAIL':
                    tempStatus['overall'] = 'WARNING'
                tempStatus['warnings'].append(f"{sensorName} MAX: {maxTemp}°C")
                if sensorName not in tempStatus['warningSensors']:
                    tempStatus['warningSensors'].append(sensorName)
                tempStatus['maxTempIssues'].append(f"{sensorName}: {maxTemp}°C")
                tempStatus['sensorDetails'][sensorName]['maxStatus'] = 'WARNING'
                self.logger.warning(f"! WARNING MAX: {sensorName} peaked at {maxTemp}°C (threshold: {self.TEMP_WARNING_THRESHOLD}°C)")
                
            else:
                self.logger.info(f"✓ {sensorName} MAX: {maxTemp}°C - OK")
            
            # Evaluate average temperatures
            if avgTemp >= AVG_TEMP_CRITICAL:
                tempStatus['overall'] = 'FAIL'
                tempStatus['critical'].append(f"{sensorName} AVG: {avgTemp}°C")
                if sensorName not in tempStatus['criticalSensors']:
                    tempStatus['criticalSensors'].append(sensorName)
                tempStatus['avgTempIssues'].append(f"{sensorName}: {avgTemp}°C")
                tempStatus['sensorDetails'][sensorName]['avgStatus'] = 'CRITICAL'
                self.logger.error(f"✗ CRITICAL AVG: {sensorName} sustained {avgTemp}°C (threshold: {AVG_TEMP_CRITICAL}°C)")
                
            elif avgTemp >= AVG_TEMP_WARNING:
                if tempStatus['overall'] != 'FAIL':
                    tempStatus['overall'] = 'WARNING'
                tempStatus['warnings'].append(f"{sensorName} AVG: {avgTemp}°C")
                if sensorName not in tempStatus['warningSensors']:
                    tempStatus['warningSensors'].append(sensorName)
                tempStatus['avgTempIssues'].append(f"{sensorName}: {avgTemp}°C")
                tempStatus['sensorDetails'][sensorName]['avgStatus'] = 'WARNING'
                self.logger.warning(f"! WARNING AVG: {sensorName} sustained {avgTemp}°C (threshold: {AVG_TEMP_WARNING}°C)")
                
            else:
                self.logger.info(f"✓ {sensorName} AVG: {avgTemp}°C - OK")
        
        # Log summary
        self.logger.info(f"Temperature evaluation: {tempStatus['overall']}")
        self.logger.info(f"  Peak temperature: {tempStatus['maxTemp']}°C")
        self.logger.info(f"  Highest sustained average: {tempStatus['maxAvgTemp']}°C")
        self.logger.info(f"  Thresholds - Max: {self.TEMP_WARNING_THRESHOLD}°C/{self.TEMP_CRITICAL_THRESHOLD}°C, Avg: {AVG_TEMP_WARNING}°C/{AVG_TEMP_CRITICAL}°C")
        
        return tempStatus
    
    def evaluateCpuSpeed(self, speedSummary):
        # Analyze CPU speed performance against expected limits for pass/fail status
        speedStatus = {
            'overall': 'PASS',
            'issues': [],
            'expectedMin': None,
    ...
    def getMonitoringSummary(self):
        # Calculate statistics from collected monitoring data (CPU speeds and temperatures)
        summary = {}
    ...
    def getOverallTestStatus(self):
        # Determine final test result based on all individual test outcomes
        statuses = []
    ...
    def openResultsFile(self):
        """Open the results file in a GUI text editor automatically after completion (Linux only)"""
        try:
            # First try xdg-open (opens with default GUI application)
            try:
                subprocess.run(['xdg-open', str(self.outputFile)], 
                             check=True, 
                             timeout=5,
                             stdout=subprocess.DEVNULL, 
                             stderr=subprocess.DEVNULL)
                self.logger.info(f"Opened results file with default GUI application: {self.outputFile}")
                return
            except (subprocess.CalledProcessError, subprocess.TimeoutExpired, FileNotFoundError):
                pass
            
            # Fallback to specific GUI text editors (no console editors)
            gui_editors = ['gedit', 'kate', 'mousepad', 'leafpad', 'pluma', 'xed', 'sublime_text', 'code']
            ...
            for editor in gui_editors:
                try:
                    ...
                    subprocess.Popen([editor, str(self.outputFile)],
                                   stdout=subprocess.DEVNULL, 
                                   stderr=subprocess.DEVNULL)
                    self.logger.info(f"Opened results file with {editor}: {self.outputFile}")
                    return
                    
                except (subprocess.CalledProcessError, FileNotFoundError, subprocess.TimeoutExpired):
                    continue
            
            # If no GUI editor found, just log the location
            self.logger.info(f"No GUI text editor found. Results file created: {self.outputFile}")
            self.logger.info("Please open the file manually to view results")
            
        except Exception as e:
            self.logger.warning(f"Could not auto-open results file: {e}")
            self.logger.info(f"Results saved to: {self.outputFile}")
    
    def writeOutputFile(self):
        # Generate comprehensive test results report with clear pass/fail status (no duplication)
        try:
            with open(self.outputFile, 'w') as f:
                ...
                # Temperature Status with detailed breakdown
                if 'temperatureStatus' in self.testResults:
                    ...
                    f.write(f"  Peak Temperature: {tempStatus['maxTemp']}°C\n")
                    f.write(f"  Highest Sustained Avg: {tempStatus['maxAvgTemp']}°C\n")
                    f.write(f"  Max Temp Thresholds: Warning {self.TEMP_WARNING_THRESHOLD}°C, Critical {self.TEMP_CRITICAL_THRESHOLD}°C\n")
                    f.write(f"  Avg Temp Thresholds: Warning {self.TEMP_WARNING_THRESHOLD-2.5}°C, Critical {self.TEMP_CRITICAL_THRESHOLD-2.5}°C\n")
                    
                    if tempStatus.get('critical'):
                        f.write(f"  CRITICAL ISSUES:\n")
                        for issue in tempStatus['critical']:
                            f.write(f"    - {issue}\n")
                    
                    if tempStatus.get('warnings'):
                        f.write(f"  WARNING ISSUES:\n")
                        for issue in tempStatus['warnings']:
                            f.write(f"    - {issue}\n")
                    
                    # Detailed sensor breakdown (ONLY ONCE)
                    if tempStatus.get('sensorDetails'):
                        f.write(f"\n  TEMPERATURE SENSOR DETAILS:\n")
                        for sensor, details in tempStatus['sensorDetails'].items():
                            f.write(f"    {sensor}:\n")
                            f.write(f"      Maximum: {details['max']}°C ({details['maxStatus']})\n")
                            f.write(f"      Average: {details['avg']}°C ({details['avgStatus']})\n")
                            f.write(f"      Minimum: {details['min']}°C\n")
                f.write("\n")
                
                # CPU Performance Summary (no temperature duplication)
                if 'monitoringSummary' in self.testResults and self.testResults['monitoringSummary']:
                    ...
                    f.write("CPU PERFORMANCE SUMMARY:\n")
                    f.write("-" * 30 + "\n")
                    ...
                    f.write("CPU PERFORMANCE SUMMARY:\n")
                    f.write("-" * 30 + "\n")
                    f.write("No CPU performance data available\n")
    ...
    def runAllTests(self):
        # Execute complete test suite and generate final pass/fail report
        ...
        if 'temperatureStatus' in self.testResults:
            tempStatus = self.testResults['temperatureStatus']
            self.logger.info(f"Temperature Test: {tempStatus['overall']} (Peak: {tempStatus['maxTemp']}°C, Max Avg: {tempStatus['maxAvgTemp']}°C)")
        ...
        # Auto-open results file
        self.openResultsFile()
        
        return overallStatus in ['PASS', 'WARNING']

def main():
    # Main entry point - parse arguments and execute host testing suite

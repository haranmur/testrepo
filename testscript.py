...
- CPU benchmarking with mprime for 10 minutes
...
class HostTester:
    def __init__(self, logPath="./host_tests"):
        ...
        # Fixed benchmark duration - 10 minutes
        self.benchmarkDurationMinutes = 10
        
        # Define tool paths
        self.dgDiagPath = Path("DGDiagTools_Internal")
        ...
        self.logFile = self.logPath / f"HostTest_{timestamp}.log"
        
        # Create hostname-based output file
        hostname = platform.node().replace('.', '_').replace('-', '_')
        self.outputFile = self.logPath / f"{hostname}_test_results.txt"
    ...
    def verifyToolPaths(self):
        ...
        # Check DGDiagTool
        dgDiagExe = self.dgDiagPath / "DGDiagTool.exe"
        if not dgDiagExe.exists():
            self.logger.error(f"DGDiagTool.exe not found at: {dgDiagExe}")
            self.logger.error(f"Expected path: {self.dgDiagPath.absolute()}")
        else:
            self.logger.info(f"Found DGDiagTool.exe at: {dgDiagExe.absolute()}")
    ...
    def runDgDiag(self, command, timeout=30):
        ...
        if not dgDiagExe.exists():
            errorMsg = f"DGDiagTool.exe not found at: {dgDiagExe.absolute()}"
        ...
        fullCommand = [str(dgDiagExe), command]
    ...
    def runStressNgFallback(self):
        """Fallback benchmark using stress-ng for 10 minutes"""
    ...
    def testPcieSlots(self):
        ...
        try:
            ...
            if speedResult["success"]:
                try:
                    speedMatch = re.search(r'(\d+\.?\d*)', speedResult["output"])
                    if speedMatch:
                        pcieResults["actualSpeed"] = float(speedMatch.group(1))
                        self.logger.info(f"Current Link Speed: {pcieResults['actualSpeed']} GT/s")
            ...
            if maxSpeedResult["success"]:
                try:
                    speedMatch = re.search(r'(\d+\.?\d*)', maxSpeedResult["output"])
                    if speedMatch:
                        pcieResults["expectedSpeed"] = float(speedMatch.group(1))
                        self.logger.info(f"Maximum Link Speed: {pcieResults['expectedSpeed']} GT/s")
                except (ValueError, AttributeError):
            ...
            if widthResult["success"]:
                try:
                    widthMatch = re.search(r'x?(\d+)', widthResult["output"])
                    if widthMatch:
            ...
            if maxWidthResult["success"]:
                try:
                    widthMatch = re.search(r'x?(\d+)', maxWidthResult["output"])
                    if widthMatch:
            ...
            if (pcieResults["actualSpeed"] is not None and 
                ...
                if pcieResults["speedMatch"]:
                    self.logger.info(f"  ✓ Speed: Current {pcieResults['actualSpeed']} GT/s matches maximum {pcieResults['expectedSpeed']} GT/s - PASS")
                else:
                    self.logger.warning(f"  ! Speed: Current {pcieResults['actualSpeed']} GT/s does not match maximum {pcieResults['expectedSpeed']} GT/s")
            else:
    ...
    def writeOutputFile(self):
        ...
        try:
            with open(self.outputFile, 'w') as f:
                ...
                f.write(f"================\n\n")
                f.write(f"Hostname: {self.testResults['hostname']}\n")
                ...
                if 'pcieResults' in self.testResults:
                    ...
                    f.write(f"PCIe Current Speed: {pcieData.get('actualSpeed', 'N/A')} GT/s\n")
                    f.write(f"PCIe Maximum Speed: {pcieData.get('expectedSpeed', 'N/A')} GT/s\n")
    ...
    def runAllTests(self):
        """Run all tests"""
        self.logger.info(f"Starting tests on {platform.node()}")
